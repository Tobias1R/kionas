...
use worker::services::query_execution::apply_filter_pipeline;
...
fn positive_int_literal_matches_int_column() {
    ...
    assert!(
        result.is_ok(),
        "int literal for int column should pass type validation"
    );
}
...
fn positive_string_literal_matches_string_column() {
    ...
    let batch = batch_mixed_types();
    let result = apply_filter_pipeline(&[batch], "name = 'Alice'", Some(&sample_schema_metadata()));
    assert!(
        result.is_ok(),
        "string literal for string column should pass type validation"
    );
}
...
fn positive_bool_literal_matches_bool_column() {
    ...
    let batch = batch_mixed_types();
    let result = apply_filter_pipeline(&[batch], "active = true", Some(&sample_schema_metadata()));
    assert!(
        result.is_ok(),
        "bool literal for bool column should pass type validation"
    );
}
...
fn positive_integer_comparisons() {
    ...
    let comparisons = vec![
        "id = 1", "id != 2", "id > 0", "id >= 1", "id < 10", "id <= 5",
    ];

    for predicate in comparisons {
        let batch = batch_mixed_types();
        let result = apply_filter_pipeline(&[batch], predicate, Some(&sample_schema_metadata()));
        assert!(
            result.is_ok(),
...
fn positive_is_null_on_any_column() {
    ...
    let is_null_predicates = vec!["id IS NULL", "name IS NULL", "active IS NULL"];
...
fn positive_is_not_null_on_any_column() {
    ...
    let is_not_null_predicates = vec!["id IS NOT NULL", "name IS NOT NULL", "active IS NOT NULL"];
...
fn negative_int_literal_rejects_string_column() {
    ...
    let batch = batch_mixed_types();
    let result = apply_filter_pipeline(&[batch], "name = 42", Some(&sample_schema_metadata()));
    assert!(
        result.is_err(),
...
fn negative_string_literal_rejects_int_column() {
    ...
    let batch = batch_mixed_types();
    let result = apply_filter_pipeline(&[batch], "id = 'invalid'", Some(&sample_schema_metadata()));
    assert!(
        result.is_err(),
...
fn negative_int_literal_rejects_bool_column() {
    ...
    let batch = batch_mixed_types();
    let result = apply_filter_pipeline(&[batch], "active = 123", Some(&sample_schema_metadata()));
    assert!(
        result.is_err(),
...
fn error_message_contains_column_name() {
    ...
    if let Err(error_msg) = result {
        assert!(
            error_msg.contains("name") || error_msg.to_lowercase().contains("type"),
