...
impl MinioProvider {
    ...
    pub async fn new(cfg: MinioConfig) -> Result<Self, Box<dyn std::error::Error + Send + Sync>> {
        ...
        if let (Some(access), Some(secret)) = (cfg.access_key.clone(), cfg.secret_key.clone()) {
            unsafe {
                std::env::set_var("AWS_ACCESS_KEY_ID", access.clone());
            }
            unsafe {
                std::env::set_var("AWS_SECRET_ACCESS_KEY", secret.clone());
            }
            // Set endpoint
            unsafe {
                std::env::set_var("AWS_ENDPOINT_URL", cfg.endpoint.clone().unwrap_or_default());
            }
            // Set region
            unsafe {
                std::env::set_var("AWS_REGION", region.as_ref());
            }
            if let Some(token) = cfg.access_key.as_ref() {
                // no-op if none
        ...
        if let Some(endpoint) = cfg.endpoint.as_ref() {
            ...
            log::info!(
                "Configured Minio endpoint and enabled path-style: {}",
                endpoint
            );
        }
