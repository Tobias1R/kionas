...
pub(crate) async fn execute_query_task(
    ...
    let mut batches = load_scan_batches(shared, &source_prefix).await?;
...
fn extract_runtime_plan(task: &worker_service::Task) -> Result<RuntimePlan, String> {
    ...
    for op in physical_plan.operators {
        match op {
            PhysicalOperator::TableScan { .. } | PhysicalOperator::Materialize => {}
    ...
    if projection_exprs.is_empty() {
        return Err("physical projection expressions are empty".to_string());
    }
...
mod tests {
    use super::{
        ...
        decode_parquet_batches, encode_result_metadata, parse_projection_identifier,
        source_table_staging_prefix, split_case_insensitive,
    };
