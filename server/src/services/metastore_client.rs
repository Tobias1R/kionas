...
impl MetastoreClient {
    ...
    pub async fn connect_with_shared(shared: &SharedData) -> Result<Self, Box<dyn Error + Send + Sync>> {
    ...
    pub async fn execute(&mut self, req: MetastoreRequest) -> Result<MetastoreResponse, Box<dyn Error + Send + Sync>> {
