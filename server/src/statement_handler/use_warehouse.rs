...
pub async fn handle_use_warehouse(query: &str, session_id: &str, shared_data: &SharedData) -> Result<String, String> {
    ...
    match session_opt {
        Some(mut s) => {
            ...
            let state = shared_data.lock().await;
            state.session_manager.update_session(session_id.to_string(), &mut s).await;
