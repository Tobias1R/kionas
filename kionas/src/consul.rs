...
pub async fn download_cluster_info(
    consul_url: &str,
...
pub async fn download_worker_info(
    consul_url: &str,
    worker_id: &str,
) -> Result<WorkerInfo, Box<dyn std::error::Error + Send + Sync>> {
    let client = Client::new();
    let url = format!(
        "{}/v1/kv/{}{}?raw",
        consul_url, CONSUL_NODE_CONFIG_PREFIX, worker_id
    );
