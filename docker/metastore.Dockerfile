...
# Install Rust components required by project quality gates.
...
# Add minimal placeholder targets so workspace members are valid during cargo fetch.
RUN touch kionas/src/lib.rs \
...
# Copy only workspace members and required files for development and testing.
COPY kionas ./kionas
COPY server ./server
COPY worker ./worker
COPY client ./client
