use rcgen::{date_time_ymd, Certificate, CertificateParams, DistinguishedName, DnType, KeyPair, SanType};
use std::{fs, vec};
use pem;
...
fn generate_cert(name: &str) -> Result<(), Box<dyn std::error::Error>>{
	...
	fs::write(format!("certs/{name}_key.pem"), key_pair.serialize_pem().as_bytes())?;
...
fn issue_cert(name: &str, 
		issuer_pem: &Certificate,
		issuer_key: &KeyPair) -> Result<(), Box<dyn std::error::Error>> {
		let mut params: CertificateParams = Default::default();
		params.not_before = date_time_ymd(1975, 1, 1);
		params.not_after = date_time_ymd(4096, 1, 1);
		params.distinguished_name = DistinguishedName::new();
