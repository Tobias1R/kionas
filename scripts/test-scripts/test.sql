...
CREATE DATABASE IF NOT EXISTS jambaxxx;

CREATE SCHEMA IF NOT EXISTS jambaxxx.test_schema;

-- Create test table with various data types for comprehensive testing
CREATE TABLE IF NOT EXISTS jambaxxx.test_schema.products (
    id INT,
    name VARCHAR(100),
    price INT,
    quantity INT,
...
INSERT INTO jambaxxx.test_schema.products (id, name, price, quantity, category, description, discount_rate, is_active) VALUES
...
SELECT 'Phase 9a Test 1: NULL names' AS test_name, id, name FROM jambaxxx.test_schema.products WHERE name IS NULL;

SELECT * FROM jambaxxx.test_schema.products;

-- Test 2: Find products with non-NULL names
SELECT id, name FROM jambaxxx.test_schema.products WHERE name IS NOT NULL;

-- Test 3: Find products with NULL descriptions
SELECT 'Phase 9a Test 3: NULL descriptions' AS test_name, id, description FROM jambaxxx.test_schema.products WHERE description IS NULL;

-- Test 4: Find products with non-NULL descriptions
SELECT 'Phase 9a Test 4: Non-NULL descriptions' AS test_name, id, description FROM jambaxxx.test_schema.products WHERE description IS NOT NULL;

-- Test 5: Find products with NULL quantity (combined with other predicates)
SELECT 'Phase 9a Test 5: NULL quantity' AS test_name, id, name, quantity FROM jambaxxx.test_schema.products WHERE quantity IS NULL;

-- Test 6: Find active products (non-NULL is_active AND is_active = true)
SELECT 'Phase 9a Test 6: Active products' AS test_name, id, name, is_active FROM jambaxxx.test_schema.products WHERE is_active IS NOT NULL;
...
FROM jambaxxx.test_schema.products 
...
SELECT id, name, price FROM jambaxxx.test_schema.products WHERE price = 150;

-- Test 9: Type matching - greater than (price > 100)
SELECT 'Phase 9b Test 9: Price greater than 100' AS test_name, id, name, price FROM jambaxxx.test_schema.products WHERE price > 100;

-- Test 10: Type matching - less than (price < 100)
SELECT 'Phase 9b Test 10: Price less than 100' AS test_name, id, name, price FROM jambaxxx.test_schema.products WHERE price < 100;

-- Test 11: Type matching - less than or equal (price <= 75)
SELECT 'Phase 9b Test 11: Price <= 75' AS test_name, id, name, price FROM jambaxxx.test_schema.products WHERE price <= 75;

-- Test 12: Type matching - greater than or equal (price >= 200)
SELECT 'Phase 9b Test 12: Price >= 200' AS test_name, id, name, price FROM jambaxxx.test_schema.products WHERE price >= 200;

-- Test 13: Type matching - not equal (id != 1)
SELECT 'Phase 9b Test 13: ID not equal to 1' AS test_name, id, name FROM jambaxxx.test_schema.products WHERE id != 1;

-- Test 14: String comparison (category = 'Electronics')
SELECT 'Phase 9b Test 14: Category equals Electronics' AS test_name, id, name, category FROM jambaxxx.test_schema.products WHERE category = 'Electronics';

-- Test 15: String comparison with non-match
SELECT 'Phase 9b Test 15: Category not Furniture' AS test_name, id, name, category FROM jambaxxx.test_schema.products WHERE category != 'Furniture';
...
SELECT COUNT(*) as total_rows FROM jambaxxx.test_schema.products WHERE price > 0 AND price < 1000;

-- ============================================================================
-- Cleanup (optional - uncomment to reset)
-- ============================================================================
-- DROP TABLE jambaxxx.test_schema.products;
-- DROP SCHEMA jambaxxx.test_schema;
-- DROP DATABASE jambaxxx;
...
-- UNION
select id, name from bench.seed1.customers where id <= 5 union select id, name from bench.seed1.customers where id > 15 and id <= 20 order by id;

-- union all jambaxxx2
SELECT id, name, category FROM jambaxxx.test_schema.products WHERE category IN ('Electronics', 'Furniture') UNION ALL SELECT id, name, category FROM jambaxxx.test_schema.products WHERE category IN ('Accessories', 'Lighting') ORDER BY id;